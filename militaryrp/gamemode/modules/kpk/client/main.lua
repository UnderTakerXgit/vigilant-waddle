...
netstream.Hook('KPK::Bootstrap:OK', function(payload)
    ...
    frame.OnRemove = function()
        if KPK._active and IsValid(KPK.UI) == false then
            local k = KPK.KeyFor(KPK._active.cat, KPK._active.ch)
    ...
    closeBtn:SetSize(32,32); closeBtn:SetPos(frame:GetWide()-40,10); closeBtn:ClearPaint(); KPK.BtnNoText(closeBtn)
    ...
    local gearBtn = TDLib('DButton', gearBar) gearBtn:Dock(FILL) gearBtn:DockMargin(8,8,8,8) gearBtn:ClearPaint(); KPK.BtnNoText(gearBtn)
    ...
    local function mkTab(txt, val)
        local b = TDLib('DButton', modeTabs) b:Dock(LEFT) b:SetWide(120) b:DockMargin(8,10,0,10) b:ClearPaint(); KPK.BtnNoText(b)
    ...
    local function updateReplyBar()
        ...
        txt:Text('Ответ на #'..tostring(KPK._reply.id)..'  ['..stamp..']  '..KPK.Truncate(KPK._reply.content, 80), 'font_sans_18', Color(210,210,215), TEXT_ALIGN_LEFT, 10)
        local cancel = TDLib('DButton', replyBar) cancel:Dock(RIGHT) cancel:SetWide(42) cancel:ClearPaint(); KPK.BtnNoText(cancel)
    ...
    sendBtn:Dock(RIGHT)
    sendBtn:SetWide(130)
    sendBtn:ClearPaint(); KPK.BtnNoText(sendBtn)
    ...
    local function doSend()
        ...
        KPK._drafts[KPK.KeyFor(KPK._active.cat, KPK._active.ch)] = ''
        entry:SetText('')
        entry._showph = true
        KPK._reply = nil
        updateReplyBar()
    ...
    entry.OnValueChange = function()
        ...
        if KPK._active then
            ...
            if not KPK._draftSaveAt or KPK._draftSaveAt < CurTime() then
                ...
                timer.Create('KPK::DraftSave', 0.7, 1, function() KPK.SaveDrafts() end)
            end
        end
    end
    ...
    local function addMessage(p, highlightId, animated)
        ...
        if tonumber(p.is_announce or 0) == 1 then
            ...
            local url = KPK.ExtractFirstUrl(tostring(p.content or ""))
            if url then KPK.BuildLinkPreview(card, url) end
        ...
        local isMention = KPK.IsMentionedForLocal(p.content or '', p.category)
        ...
        if tonumber(p.reply_to or 0) and tonumber(p.reply_to or 0) > 0 then
            ...
            txtp:Text('Ответ на #'..tostring(p.reply_to)..'  ['..stampR..']  '..KPK.Truncate(preview, 80), 'font_sans_16', Color(210,210,215), TEXT_ALIGN_LEFT, 12)
        ...
        local url = KPK.ExtractFirstUrl(tostring(p.content or ""))
        if url then KPK.BuildLinkPreview(row, url) end
    ...
    local function requestOlder()
        ...
        local key = KPK.KeyFor(KPK._active.cat, KPK._active.ch)
    ...
    local function load(cat, ch, before, highlightId)
        ...
        if KPK._active and IsValid(entry) then
            KPK._drafts[KPK.KeyFor(KPK._active.cat, KPK._active.ch)] = entry:GetValue() or ''
            KPK.SaveDrafts()
        end
    ...
    netstream.Hook('KPK::Fetch:OK', function(r)
        ...
        local key = KPK.KeyFor(r.category, r.channel)
        ...
        if pinfo and pinfo.message_id then
            ...
            local goBtn = TDLib('DButton', pinBar) goBtn:Dock(RIGHT) goBtn:SetWide(150) goBtn:ClearPaint(); KPK.BtnNoText(goBtn)
    ...
    netstream.Hook('KPK::Pin:Update', function(r)
        if not IsValid(KPK.UI) then return end
        if KPK._active and KPK._active.cat == r.category and KPK._active.ch == r.channel then
            netstream.Start('KPK::Fetch', { category = r.category, channel = r.channel, before_id = KPK._minId[KPK.KeyFor(r.category, r.channel)] or 0 })
        end
    ...
    local function drawCats()
        ...
        for catId, def in pairs(cats) do
            ...
            for _, ch in ipairs(def.channels or {}) do
                ...
                btn:Dock(TOP) btn:SetTall(36) btn:ClearPaint() btn:DockMargin(10,6,10,0) btn.Text = ch.name; KPK.BtnNoText(btn)
    ...
    local function renderTasks(category, tasks)
        ...
        createBtn:Dock(LEFT) createBtn:SetWide(170) createBtn:DockMargin(10,8,10,8) createBtn:ClearPaint(); KPK.BtnNoText(createBtn)
    ...
    reportBtn:Dock(RIGHT) reportBtn:SetWide(170) reportBtn:DockMargin(0,10,10,10) reportBtn:ClearPaint(); KPK.BtnNoText(reportBtn)
