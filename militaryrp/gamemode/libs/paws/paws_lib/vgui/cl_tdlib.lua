...
TDLibUtil.HoverFuncChild = function(s) return s:IsHovered() or s:IsChildHovered() end
...
classes.SetupTransition = function(pnl, name, speed, fn)
	...
	pnl:On('Think', function(s)
		s[name] = Lerp(FrameTime()*speed, s[name], fn(s) && 1 or 0)
	end)
end
...
classes.FadeHover = function(pnl, col, speed, rad)
	...
	pnl:SetupTransition('FadeHoverTrans', speed, TDLibUtil.HoverFunc)
	pnl:On('Paint', function(s, w, h)
		local col = ColorAlpha(col, col.a*s.FadeHoverTrans)
...
classes.CircleFadeHover = function(pnl, col, speed)
	...
	pnl:SetupTransition('CircleFadeHoverTrans', speed, TDLibUtil.HoverFunc)
	pnl:On('Paint', function(s, w, h)
		draw.NoTexture()
		surface.SetDrawColor(ColorAlpha(col, col.a*s.CircleFadeHoverTrans))
		drawCircle(w * 0.5, h * 0.5, math.min(w, h) * 0.5)
	end)
end
...
classes.Gradient = function(pnl, col, dir, frac, op)
	...
	pnl:On('Paint', function(s, w, h)
		...
		if(dir == LEFT) then
			...
			surface.SetMaterial(op && gradRight or gradLeft)
		elseif(dir == TOP) then
			local prog = math.Round(h*frac)
			x, y, gw, gh = 0, 0, w, prog
			surface.SetMaterial(op && gradDown or gradUp)
		elseif(dir == RIGHT) then
			local prog = math.Round(w*frac)
			x, y, gw, gh = w-prog, 0, prog, h
			surface.SetMaterial(op && gradLeft or gradRight)
		elseif(dir == BOTTOM) then
			local prog = math.Round(h*frac)
			x, y, gw, gh = 0, h-prog, w, prog
			surface.SetMaterial(op && gradUp or gradDown)
		end
...
classes.SmartStick = function(pnl, dock, dockLeft, dockTop, dockRight, dockBottom, dontInvalidate)
	dock = dock or FILL
	dockLeft = dockLeft or 0
	dockTop = dockTop or 0
	dockRight = dockRight or 0
	dockBottom = dockBottom or 0
