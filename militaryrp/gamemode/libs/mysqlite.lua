...
-- Все хуйня local moo, tmsql = file.Exists("bin/gmsv_mysqloo_*.dll", "LUA"), file.Exists("bin/gmsv_tmysql4_*.dll", "LUA") все ваши bin файлы, (VoLVeR VOR)
local debug = debug
local error = error
...
local function loadMySQLModule()
    ...
    local moo, tmsql = file.Exists("lua/bin/gmsv_mysqloo_linux.dll", "GAME"), file.Exists("lua/bin/gmsv_tmysql4_linux.dll", "GAME")
